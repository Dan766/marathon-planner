...
import { generateScheduledPlan, getWorkoutLabel, getWorkoutColor, formatDate, formatDistance } from '../lib/plan-engine'
...
export function PlanViewPage() {
  ...
  const { planConfig, completedWorkouts, toggleWorkout, unit } = useStore()
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500 dark:text-gray-400 mb-4">No plan configured yet.</p>
          <button
  ...
  const handleExportICS = () => {
    const ics = generateICS(scheduledPlan, unit)
    if (ics) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Action Bar */}
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 py-3 px-4 no-print">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 py-4 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Training Progress</span>
          ...
          <div className="flex justify-between mt-2 text-xs text-gray-500 dark:text-gray-400">
            <span>Total: {formatDistance(scheduledPlan.totalMileage, unit)}</span>
            <span>
      ...
      <div className="print-only px-4 py-4">
        ...
        <p className="text-gray-600">
          {formatDate(scheduledPlan.startDate)} &mdash; {formatDate(scheduledPlan.raceDate)} | Total: {formatDistance(scheduledPlan.totalMileage, unit)}
        </p>
      </div>
      ...
      <main className="flex-1 px-4 py-6">
        <div className="max-w-6xl mx-auto space-y-4">
          {/* Day Headers (desktop) */}
          <div className="hidden lg:grid grid-cols-8 gap-2 text-center text-sm font-medium text-gray-500 dark:text-gray-400 px-2">
            <div>Week</div>
            {dayLabels.map((d) => (
              <div key={d}>{d}</div>
            ))}
...
function WeekRowComponent({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      {/* Days Grid - Desktop */}
      <div className="hidden lg:grid grid-cols-7 gap-px bg-gray-100 dark:bg-gray-700">
        {week.days.map((day) => {
          ...
          return (
            <button
              ...
              className={`p-3 text-center transition-all ${
                day.workout.type === 'rest' ? 'bg-gray-50 dark:bg-gray-900 cursor-default' : 'bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer'
            ...
            >
              ...
              <div className={`text-xs font-medium px-2 py-1 rounded border ${getWorkoutColor(day.workout.type)} ${
                ...
                {getWorkoutLabel(day.workout, unit)}
              </div>
              {completed && (
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all ${
                day.workout.type === 'rest' ? 'cursor-default' : 'hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer active:bg-gray-100 dark:active:bg-gray-600'
              } ${day.isToday ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}
            >
              <div className="w-10 text-center">
                <div className="text-xs text-gray-400">{dayLabels[i]}</div>
                <div className="text-sm font-medium text-gray-600 dark:text-gray-300">{formatDate(day.date)}</div>
              </div>
