...
export function StepPreview() {
  ...
  if (!scheduledPlan || !planConfig) {
    return <p className="text-center text-gray-500">Configure your plan first.</p>
  }

  return (
    <div>
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-900">Plan Preview</h2>
        <p className="text-gray-500 mt-2">
          Review your complete {scheduledPlan.plan.name} training schedule
        </p>
        <p className="text-sm text-gray-400 mt-1">
          {formatDate(scheduledPlan.startDate)} &mdash; {formatDate(scheduledPlan.raceDate)} | {Math.round(scheduledPlan.totalMileage)} total miles
        </p>
      </div>
              ...
              {week.days.map((day, i) => (
                ...
                >
                  ...
                  <div className={`text-xs font-medium px-1 py-1 rounded border ${getWorkoutColor(day.workout.type)}`}>
                    <span className="hidden sm:inline">{getWorkoutLabel(day.workout)}</span>
                    <span className="sm:hidden">
                      {day.workout.type === 'rest' ? 'R' : day.workout.distance ? `${day.workout.distance}` : day.workout.type.charAt(0).toUpperCase()}
                    </span>
                  </div>
                </div>
              ))}
